...
if (isset($_GET["action"]) && $_GET["action"] == "suma") {
    ...
    <table>
        ...
        <tr>
            <td>Total :</td>
            <td><h3> <?php echo $calculadora["total"] ?></h3></td>
        </tr>
    </table>
    <?php
}
...
if (isset($_GET["action"]) && $_GET["action"] == "resta") {
    $calculadora = file_get_contents(
        'http://localhost/practicas_progweb_unidad05/Calculadora2/servicio.php?action=resta&a=' . $_GET["a"] . '&b=' . $_GET["b"]);
    $calculadora = json_decode($calculadora, true);
    ?>
...
if (isset($_GET["action"]) && $_GET["action"] == "division") {
    $calculadora = file_get_contents(
        'http://localhost/practicas_progweb_unidad05/Calculadora2/servicio.php?action=division&a=' . $_GET["a"] . '&b=' . $_GET["b"]);
    $calculadora = json_decode($calculadora, true);
    ?>
